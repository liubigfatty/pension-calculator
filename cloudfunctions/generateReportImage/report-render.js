...
const { createCanvas, loadImage: _loadImage, GlobalFonts } = require('@napi-rs/canvas')
const QR_PATH = path.join(__dirname, 'assets', 'consult-wechat.jpg')
...
// 异步加载图片（二维码），失败返回 null（@napi-rs/canvas 顶层 loadImage 接受文件路径）
function loadImage(src) {
  if (!src) return Promise.resolve(null)
  return _loadImage(src).catch(function () { return null })
}

// 二维码只加载一次（模块级单例），避免每次请求重复读磁盘
let _qrPromise = null
function getQrImage() {
  if (!_qrPromise) {
    _qrPromise = loadImage(QR_PATH).catch(function () { return null })
  }
  return _qrPromise
}
...
// 矢量绘制人民币符号 ¥（不依赖字体字形，避免黑体对半角/全角支持不一致导致 tofu）
function drawYenSymbol(ctx, x, y, size, color, lineWidth) {
  ctx.save()
  ctx.strokeStyle = color
  ctx.lineWidth = lineWidth || Math.max(2, Math.round(size / 16))
  ctx.lineCap = 'round'
  ctx.lineJoin = 'round'
  const s = size
  // 垂直主杆
...
// 绘制带人民币符号的金额（自动识别 ¥/￥ 前缀，字体无关，返回总宽度）
// 仅 mode === 'draw' 时落笔；measure 阶段只返回预估宽度（0）
function drawCurrencyAmount(ctx, str, x, y, font, color, align, symbolSize, lineWidth, mode) {
  if (mode !== 'draw') return 0
  ...
  const sz = symbolSize || Math.round((parseFloat(String(font).match(/\d+/)?.[0]) || 24) * 0.75)
  const symW = sz * 0.55
  const gap = Math.max(4, Math.round(sz * 0.20))
  const totalW = symW + gap + numW
  let startX
  if (align === 'center') startX = x - totalW / 2
  else if (align === 'right') startX = x - totalW
  else startX = x
  drawYenSymbol(ctx, startX + symW / 2, y, sz, color, lineWidth)
  ctx.textAlign = 'left'
  ctx.fillText(numStr, startX + symW + gap, y)
  return totalW
}
...
function section(ctx, yTop, num, title, bodyFn, mode, data) {
  ...
  let bodyH
  if (mode === 'measure') {
    bodyH = bodyFn(ctx, CONTENT_X, contentY, 'measure', data)
  } else {
    // draw 模式：先 measure 一次拿高度（不绘制），再画卡片，最后只绘制一次 body，杜绝叠字
    bodyH = bodyFn(ctx, CONTENT_X, contentY, 'measure', data)
    ...
    ctx.fill()
    drawText(ctx, num, PAD + 25, yTop + 21, "700 11px 'SimHei'", '#fff', 'center', 'draw')
    drawText(ctx, title, PAD + 14 + 22 + 8, yTop + 21, "700 15px 'SimHei'", '#333', 'left', 'draw')
...
function bodyModule1(ctx, x, y, mode, data) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (mode === 'draw') {
      ...
      ctx.fill()
      drawText(ctx, items[i][0], cx + cellW / 2, cy + 13, "400 11px 'SimHei'", '#888', 'center', 'draw')
      drawText(ctx, String(items[i][1]), cx + cellW / 2, cy + 32, "600 14px 'SimHei'", items[i][2] ? GOLD : '#333', 'center', 'draw')
    }
  }
...
function bodyModule2(ctx, x, y, mode, data) {
  ...
  if (mode === 'draw') {
    ...
    ctx.fill()
    drawText(ctx, '正常退休', x + cardW / 2, yy + 18, "400 12px 'SimHei'", '#666', 'center', 'draw')
    drawCurrencyAmount(ctx, data.legalTotalStr || (data.legalTotal || ''), x + cardW / 2, yy + 44, "700 24px 'SimHei'", '#333', 'center', 20, 2, 'draw')
    drawText(ctx, '/月 · ' + (data.legalAgeShow || '') + '起领', x + cardW / 2, yy + 70, "400 11px 'SimHei'", '#999', 'center', 'draw')
  }
  if (data.hasEarly) {
    ...
    if (mode === 'draw') {
      ...
      ctx.fill()
      drawText(ctx, '弹性提前退休', ex + cardW / 2, yy + 18, "400 12px 'SimHei'", '#666', 'center', 'draw')
      drawCurrencyAmount(ctx, data.flexTotalStr || (data.flexTotal || ''), ex + cardW / 2, yy + 44, "700 24px 'SimHei'", GREEN_TXT, 'center', 20, 2, 'draw')
      drawText(ctx, '/月 · ' + (data.flexAgeShow || '') + '起领', ex + cardW / 2, yy + 70, "400 11px 'SimHei'", '#999', 'center', 'draw')
    }
  }
  yy += cardH + 10
  ...
  if (mode === 'draw') {
    ...
    ctx.fill()
    drawText(ctx, headers[0], c0 + 6, yy + rowH / 2, "600 12px 'SimHei'", '#666', 'left', 'draw')
    drawText(ctx, headers[1], c1 + colW[1] / 2, yy + rowH / 2, "600 12px 'SimHei'", '#666', 'center', 'draw')
    drawText(ctx, headers[2], c2 + colW[2] / 2, yy + rowH / 2, "600 12px 'SimHei'", '#4A8C5C', 'center', 'draw')
  }
  yy += rowH
  for (let i = 0; i < rowsData.length; i++) {
    ...
    if (mode === 'draw') {
      drawText(ctx, rowsData[i][0], c0 + 6, yy + rowH / 2, "400 12px 'SimHei'", '#666', 'left', 'draw')
      drawText(ctx, String(rowsData[i][1] == null ? '--' : rowsData[i][1]), c1 + colW[1] / 2, yy + rowH / 2, "600 12px 'SimHei'", '#333', 'center', 'draw')
      drawText(ctx, String(rowsData[i][2] == null ? '--' : rowsData[i][2]), c2 + colW[2] / 2, yy + rowH / 2, "600 12px 'SimHei'", '#4A8C5C', 'center', 'draw')
    }
    yy += rowH
  }
  return yy - y
...
function bodyModule3(ctx, x, y, mode, data) {
  ...
  yy += drawWrapped(ctx, cap, x, yy, CONTENT_W, 16, "400 11px 'SimHei'", GOLD, 'left', mode)
  yy += 4

  const colW = [112, 66, 66, 67]
  ...
  if (mode === 'draw') {
    ...
    roundRectPath(ctx, x, yy, CONTENT_W, rowH, 4)
    ctx.fill()
    drawText(ctx, '领到年龄', c0 + 6, yy + rowH / 2, "600 12px 'SimHei'", '#666', 'left', 'draw')
    drawText(ctx, '正常退休', c1 + colW[1] / 2, yy + rowH / 2, "600 12px 'SimHei'", '#666', 'center', 'draw')
    drawText(ctx, '弹性提前', c2 + colW[2] / 2, yy + rowH / 2, "600 12px 'SimHei'", '#4A8C5C', 'center', 'draw')
    drawText(ctx, '差额', c3 + colW[3] / 2, yy + rowH / 2, "600 12px 'SimHei'", '#666', 'center', 'draw')
  }
  yy += rowH
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (mode === 'draw') {
      ...
      const ageLabel = it.ageLabel || (it.age + '岁')
      drawText(ctx, ageLabel, c0 + 6, yy + rowH / 2, "400 11px 'SimHei'", '#666', 'left', 'draw')
      drawText(ctx, '¥' + it.legalTotal, c1 + colW[1] / 2, yy + rowH / 2, "400 12px 'SimHei'", '#333', 'center', 'draw')
      drawText(ctx, '¥' + it.flexTotal, c2 + colW[2] / 2, yy + rowH / 2, "400 12px 'SimHei'", '#4A8C5C', 'center', 'draw')
      let diffColor = RED_TXT
      if (isBE) diffColor = GOLD
      else if (it.diff && it.diff.indexOf('+') === 0) diffColor = GREEN_TXT
      drawText(ctx, it.diff || '', c3 + colW[3] / 2, yy + rowH / 2, "400 12px 'SimHei'", diffColor, 'center', 'draw')
    }
    yy += rowH
  }
  ...
  if (data.breakEvenAge && data.breakEvenAge > 0) {
    ...
    if (mode === 'draw') {
      ...
      ctx.fill()
      drawText(ctx, '盈亏平衡点：约 ' + data.breakEvenAge + '岁 后正常退休累计领取反超', x + CONTENT_W / 2, yy + 18, "400 12px 'SimHei'", GOLD, 'center', 'draw')
    }
    yy += 36
  }
  return yy - y
...
function bodyModule4(ctx, x, y, mode, data) {
  ...
  const cardH = 18 + drawWrapped(ctx, adviceText, x, y + 10 + 18, CONTENT_W - 24, 18, "400 12px 'SimHei'", titleColor, 'left', 'measure') + 10
  if (mode === 'draw') {
    ctx.fillStyle = cardBg
    roundRectPath(ctx, x, yy, CONTENT_W, cardH, 10)
    ctx.fill()
    drawText(ctx, isWorker ? '企业职工退休分析' : '灵活就业退休分析', x + 12, yy + 10 + 9, "700 13px 'SimHei'", '#333', 'left', 'draw')
    drawWrapped(ctx, adviceText, x + 12, yy + 10 + 18, CONTENT_W - 24, 18, "400 12px 'SimHei'", titleColor, 'left', 'draw')
  }
  yy += cardH + 8
  ...
  for (let i = 0; i < featItems.length; i++) {
    ...
    const descH = drawWrapped(ctx, featItems[i][1], 0, 0, CONTENT_W - 20, 14, "400 11px 'SimHei'", '#888', 'left', 'measure')
  ...
  if (mode === 'draw') {
    ...
    ctx.fill()
    drawText(ctx, '关键差异分析', x + 10, yy + 10 + 9, "700 13px 'SimHei'", '#333', 'left', 'draw')
    let fy = yy + 10 + 18 + 8
    for (let i = 0; i < featItems.length; i++) {
      drawText(ctx, featItems[i][0], x + 10, fy + 8, "700 12px 'SimHei'", '#333', 'left', 'draw')
      fy += 8 + 12 + 8
      fy += drawWrapped(ctx, featItems[i][1], x + 10, fy, CONTENT_W - 20, 14, "400 11px 'SimHei'", '#888', 'left', 'draw')
      fy += 4
    }
  }
  yy += featBlockH + 8
  ...
  const suggestH = 6 + drawWrapped(ctx, suggestText, x, yy + 6, CONTENT_W - 20, 18, "400 11px 'SimHei'", suggestColor, 'left', 'measure') + 6
  if (mode === 'draw') {
    ctx.fillStyle = suggestBg
    roundRectPath(ctx, x, yy, CONTENT_W, suggestH, 8)
    ctx.fill()
    drawWrapped(ctx, suggestText, x + 10, yy + 6, CONTENT_W - 20, 18, "400 11px 'SimHei'", suggestColor, 'left', 'draw')
  }
  yy += suggestH + 8
  ...
  if (mode === 'draw') {
    ...
    ctx.fill()
    drawText(ctx, '养老金替代率分析', x + 10, yy + 10 + 9, "700 13px 'SimHei'", '#333', 'left', 'draw')
    let ry = yy + 10 + 18 + 4
    drawText(ctx, '月养老金', x + 10, ry + 8, "400 12px 'SimHei'", '#888', 'left', 'draw')
    drawText(ctx, data.legalTotal || '', x + CONTENT_W - 10, ry + 8, "700 12px 'SimHei'", '#333', 'right', 'draw')
    ry += 24
    drawText(ctx, '退休前月工资（按缴费基数×指数推算）', x + 10, ry + 8, "400 12px 'SimHei'", '#888', 'left', 'draw')
    drawText(ctx, '约¥' + (data.estPreRetireSalary || ''), x + CONTENT_W - 10, ry + 8, "700 12px 'SimHei'", '#333', 'right', 'draw')
    ry += 24
    ctx.strokeStyle = DIVIDER
    ctx.lineWidth = 1
    ctx.beginPath()
    ...
    ctx.stroke()
    drawText(ctx, '替代率', x + 10, ry + 8, "700 12px 'SimHei'", '#888', 'left', 'draw')
    drawText(ctx, (data.replaceRate || '') + '%', x + CONTENT_W - 10, ry + 8, "700 12px 'SimHei'", GOLD, 'right', 'draw')
    ry += 24 + 4
    drawWrapped(ctx, '替代率 = 月养老金÷退休前工资。' + (data.replaceRateDesc || ''), x + 10, ry, CONTENT_W - 20, 14, "400 11px 'SimHei'", '#888', 'left', 'draw')
  }
  yy += rateBlockH
...
function bodyModule5(ctx, x, y, mode, data) {
  ...
  if (isWorker) {
    ...
    const h = 8 + drawWrapped(ctx, txt, x, yy + 8, CONTENT_W - 24, 16, "400 12px 'SimHei'", BLUE, 'left', 'measure') + 8
    if (mode === 'draw') {
      ctx.fillStyle = '#F0F4F8'
      roundRectPath(ctx, x, yy, CONTENT_W, h, 10)
      ctx.fill()
      drawWrapped(ctx, txt, x + 12, yy + 8, CONTENT_W - 24, 16, "400 12px 'SimHei'", BLUE, 'left', 'draw')
    }
    yy += h + 6
  } else {
    const tip = '灵活就业可自主选择缴费档次，不同指数对养老金的影响如下（基于当前缴费年限估算）：'
    yy += drawWrapped(ctx, tip, x, yy, CONTENT_W, 12, "400 11px 'SimHei'", '#999', 'left', mode)
    yy += 6
    // 指数表
    ...
    if (mode === 'draw') {
      ...
      roundRectPath(ctx, x, yy, CONTENT_W, rowH, 4)
      ctx.fill()
      drawText(ctx, '缴费指数', c0 + 6, yy + rowH / 2, "600 12px 'SimHei'", '#666', 'left', 'draw')
      drawText(ctx, '月养老金', c1 + colW[1] / 2, yy + rowH / 2, "600 12px 'SimHei'", '#666', 'center', 'draw')
      drawText(ctx, '月增加', c2 + colW[2] / 2, yy + rowH / 2, "600 12px 'SimHei'", '#666', 'center', 'draw')
    }
    yy += rowH
    ...
    for (let i = 0; i < opts.length; i++) {
      ...
      if (mode === 'draw') {
        ...
        drawText(ctx, o.label, c0 + 6, yy + rowH / 2, "400 12px 'SimHei'", '#333', 'left', 'draw')
        drawText(ctx, o.pension, c1 + colW[1] / 2, yy + rowH / 2, "600 12px 'SimHei'", '#333', 'center', 'draw')
        const incColor = (o.inc === '--') ? '#888' : GREEN_TXT
        drawText(ctx, o.inc, c2 + colW[2] / 2, yy + rowH / 2, "400 12px 'SimHei'", incColor, 'center', 'draw')
      }
      yy += rowH
    }
    yy += 4
    const note = '注：以上为基础养老金变化估算，实际总养老金差异更大（含个人账户部分）。'
    yy += drawWrapped(ctx, note, x, yy, CONTENT_W, 12, "400 11px 'SimHei'", '#999', 'left', mode)
    yy += 6
  }
  ...
  if (mode === 'draw') {
    ...
    ctx.fill()
    ctx.stroke()
    drawText(ctx, consultTexts[0], x + 12, yy + 10 + 9, "700 12px 'SimHei'", GOLD, 'left', 'draw')
    let cy2 = yy + 10 + 18 + 8
    for (let i = 1; i < consultTexts.length; i++) {
      drawText(ctx, consultTexts[i], x + 12, cy2 + 8, "400 11px 'SimHei'", '#A08860', 'left', 'draw')
      cy2 += 16
    }
    if (data._qrImg) {
      ...
      drawText(ctx, '长按识别二维码，添加客服微信咨询', x + CONTENT_W / 2, qy + qrSize * (165 / 130) + 6 + 9, "700 12px 'SimHei'", GOLD, 'center', 'draw')
    }
  }
  yy += blockH
...
function bodyModule6(ctx, x, y, mode, data) {
  ...
  for (let i = 0; i < rows.length; i++) {
    if (mode === 'draw') {
      drawText(ctx, rows[i][0], x + 4, yy + rowH / 2, "400 13px 'SimHei'", '#333', 'left', 'draw')
      drawText(ctx, rows[i][1] || '', x + CONTENT_W - 4, yy + rowH / 2, "700 13px 'SimHei'", '#333', 'right', 'draw')
    }
    yy += rowH
    if (rows[i][2]) {
      // measure / draw 都需累加高度，仅 draw 时填充，确保长图总高与卡片背景高度一致，避免底部截断
      yy += drawWrapped(ctx, rows[i][2], x + 4, yy, CONTENT_W, 14, "400 11px 'SimHei'", '#999', 'left', mode)
    }
    if (rows[i][3] && data.extraDetails && data.extraDetails.length) {
      for (let j = 0; j < data.extraDetails.length; j++) {
        const line = data.extraDetails[j].label + ' → ' + data.extraDetails[j].amount
        const lh = 14
        const h = drawWrapped(ctx, line, x + 12, yy, CONTENT_W - 24, lh, "400 11px 'SimHei'", '#888', 'left', 'measure')
        if (mode === 'draw') {
          ctx.fillStyle = '#F8F6F4'
          roundRectPath(ctx, x + 4, yy - 2, CONTENT_W - 8, h + 6, 6)
          ctx.fill()
          drawWrapped(ctx, line, x + 12, yy, CONTENT_W - 24, lh, "400 11px 'SimHei'", '#888', 'left', 'draw')
        }
        yy += h + 6
      }
    }
  ...
  yy += 8
  if (mode === 'draw') {
    drawText(ctx, '月养老金合计', x + 4, yy + 12, "700 14px 'SimHei'", '#333', 'left', 'draw')
    drawText(ctx, (data.totalPension || '') + '/月', x + CONTENT_W - 4, yy + 12, "700 16px 'SimHei'", GOLD, 'right', 'draw')
  }
  yy += 24
  return yy - y
}
...
function drawDisclaimer(ctx, yTop, data, mode) {
  ...
  const h = 8 + drawWrapped(ctx, text, 0, yTop + 8, CARD_W - 24, 14, "400 11px 'SimHei'", '#999', 'left', 'measure') + 8
  if (mode === 'draw') {
    ctx.fillStyle = '#F8F6F4'
    roundRectPath(ctx, PAD, yTop, CARD_W, h, 8)
    ctx.fill()
    drawWrapped(ctx, text, PAD + 12, yTop + 8, CARD_W - 24, 14, "400 11px 'SimHei'", '#999', 'left', 'draw')
  }
  return h
}
...
function drawHero(ctx, yTop, data, mode) {
  ...
  if (mode === 'draw') {
    ...
    let ay = yTop + 28
    drawText(ctx, '每月可领取养老金', PAGE_W / 2, ay + 10, "400 14px 'SimHei'", 'rgba(139,115,85,0.9)', 'center', 'draw')
    ay += 20
    // Hero 金额：用矢量人民币符号 + 数字，避免字体字形问题与双符号
    drawCurrencyAmount(ctx, data.legalTotalStr || (data.legalTotal || '--'), PAGE_W / 2, ay + 28, "700 48px 'SimHei'", GOLD_LIGHT, 'center', 36, 3, 'draw')
    ay += 56
    ...
    if (hasTags) {
      ...
      drawText(ctx, tag1, PAGE_W / 2, ay + 8, "400 13px 'SimHei'", 'rgba(255,255,255,0.85)', 'center', 'draw')
      ay += 17 // tag1 line height (13 font + 4 padding)
      ay += 5 // spacing between tag lines
      drawText(ctx, tag2, PAGE_W / 2, ay + 8, "400 13px 'SimHei'", 'rgba(255,255,255,0.85)', 'center', 'draw')
    }
  }
  return amountH
}
...
function _draw(data, canvas, ctx, dpr, qrImg) {
  ...
  // 注意：canvas 尺寸由调用方在 report.js 中一次性确定（先测量总高度，再重设精确尺寸），
  // 故此处只做缩放，不再重设 width/height。
  ctx.scale(dpr, dpr)
...
// 仅测量模式：返回报告长图总高度（不实际绘制），供渲染时创建精确尺寸画布
function measure(data, ctx, dpr) {
  const d = data
  let measureY = PAD
  measureY += drawHero(ctx, measureY, d, 'measure')
  measureY += 6
...
// 渲染主入口：接收报告 data → 返回 PNG Buffer（服务端 @napi-rs/canvas，云端 Linux 已用 GlobalFonts 注册 SimHei）
async function renderReport(data) {
  // 深拷贝并把半角 ¥ 统一为全角 ￥（SimHei 对半角 ¥ 支持不好，会显示为方块）
  const cloned = JSON.parse(JSON.stringify(data))
  function replaceYen(o) {
    if (typeof o === 'string') return o.replace(/¥/g, '￥')
    if (Array.isArray(o)) return o.map(replaceYen)
    if (o && typeof o === 'object') {
      const r = {}
      for (const k in o) r[k] = replaceYen(o[k])
      return r
    }
    return o
  }
  data = replaceYen(cloned)
  const dpr = 2
  // 1) 测量总高度（临时 ctx，逻辑坐标，无需真实像素）
  const mcanvas = createCanvas(PAGE_W * dpr, 10)
  const mctx = mcanvas.getContext('2d')
  const totalH = measure(data, mctx, dpr)
  // 2) 创建精确尺寸画布并绘制（_draw 内部会 scale + 填背景 + 加载二维码）
  const canvas = createCanvas(PAGE_W * dpr, Math.ceil(totalH * dpr))
  const ctx = canvas.getContext('2d')
  data.qrPath = QR_PATH // 确保高度计算包含二维码区（实际图片由 QR_PATH 加载）
