/**
 * 批量更新所有省份的养老金计发基数数据（简化版）
 * 直接重写每个省份文件的PROV_BASE部分
 */
...
const OFFICIAL_DATA = {
  ...
  'zhejiang': {
    // 2024年官方数据：加权平均工资101194元/年 = 8433元/月
  ...
  'shandong': {
    // 2025年官方数据：计发基数7831元/月
    // 2024年官方数据：计发基数7678元/月
  ...
  'hubei': {
    // 2024年官方数据：在岗职工年平均工资89809元 = 7484元/月（全省）
  ...
  'jiangxi': {
    // 2025年官方数据：计发基数7054元/月
    // 2024年官方数据：计发基数6916元/月
...
function updateProvinceFile(provinceKey) {
  ...
  // 替换PROV_BASE对象 - 使用更简单的正则
  const provBaseStart = 'const PROV_BASE = {';
  const provBaseIndex = content.indexOf(provBaseStart);
  ...
  // 找到对应的结束括号
  let braceCount = 0;
  let endIndex = provBaseIndex + provBaseStart.length;
  for (let i = provBaseIndex + provBaseStart.length; i < content.length; i++) {
  ...
  // 替换
  const before = content.substring(0, provBaseIndex);
  const after = content.substring(endIndex);
  const newContent = before + `const PROV_BASE = ${newDataStr};` + after;
