...
function ok(label, got, want, tol) {
  ...
  if (__UM) { try { const __m = ((new Error().stack || '').split('\n')[2] || '').match(/:(\d+):\d+/); if (__m) console.log('@@UM|' + (+__m[1]) + '|' + (typeof got === 'number' ? +(got.toFixed(6)) : got)) } catch (e) {} }
...
console.log('\n【〇、基准与预算】')
ok('  起点 15年100档 月领 1899.22', +P15.total.toFixed(2),1905.56)
ok('  起点总投入 191,791', +f15.toFixed(0), 191791, 1)
ok('  30.42年100档 月领 3494.22', +P30.total.toFixed(2),3504.15)
...
ok('  路A 延年限 月领增量 1,595.00', +gainA.toFixed(2),1598.59)
...
ok('  路B 提档后月领 2,177.72', +PB.total.toFixed(2),2185.45, 0.05)
ok('  路B 月领增量 278.50', +gainB.toFixed(2),279.89, 0.05)

const EFF_Y = gainA / (BUDGET / 10000)
const EFF_T = gainB / (BUDGET / 10000)
ok('  ⭐ 每万元·延年限 388.41', +EFF_Y.toFixed(2),389.28, 0.05)
ok('  ⭐ 每万元·提档 67.82', +EFF_T.toFixed(2),68.16, 0.05)
ok('  ⭐⭐ 倍数 5.73', +(EFF_Y / EFF_T).toFixed(2),5.71, 0.02)
...
ok('  需提到 222.61 档', +(IDX_NEED * 100).toFixed(2),222.29, 0.05)
ok('  未超法定上限 300 档', IDX_NEED <= 3 ? 1 : 0, 1)
const fNeed = fee(IDX_NEED, 2010, 12)
ok('  档位路线总投入 426,951', +fNeed.toFixed(0),426335, 3)
ok('  ⭐ 比年限路线多花 194,095', +(fNeed - f30).toFixed(0),193479, 3)
...
ok('  延迟后月领 2,587.90', +DELAY.total.toFixed(2),2628.24)
ok('  ⭐ 月领增量 688.68', +dDEL.toFixed(2),722.68)
ok('  延迟后基础养老金 1,452.37', +DELAY.basicPension.amount.toFixed(2), 1452.37)
ok('  延迟后个人账户养老金 1,135.53', +DELAY.personalAccount.amount.toFixed(2),1175.87)
ok('  基础增量 304.85', +(DELAY.basicPension.amount - P15.basicPension.amount).toFixed(2), 304.85)
ok('  个人账户增量 383.83', +(DELAY.personalAccount.amount - P15.personalAccount.amount).toFixed(2),417.83)
...
ok('  ⭐ 少领三年养老金 68,372', +LOST.toFixed(0),68600, 2)
const EFF_D_A = dDEL / (feeDelay / 10000)
const EFF_D_B = dDEL / ((feeDelay + LOST) / 10000)
ok('  口径A（只算多缴）每万元 116.73', +EFF_D_A.toFixed(2),122.50, 0.05)
ok('  ⭐ 口径B（保守，含少领）每万元 54.07', +EFF_D_B.toFixed(2),56.64, 0.05)
ok('  ⭐ 保守总成本 127,367', +(feeDelay + LOST).toFixed(0),127595, 3)
...
ok('  相对值 延迟退 = 年限的 13.9%', +(EFF_D_B / EFF_Y * 100).toFixed(1),14.5, 0.2)
...
const STARTS = [['15.00年', 2010, 12, 121.4, 389.28, 68.16, 5.71],
                ['20.00年', 2005, 12, 108.4, 592.03, 76.19, 7.77],
                ['25.00年', 2000, 12, 103.0, 800.71, 85.34, 9.38]]
let prevMul = 0
...
console.log('\n【七、⭐ 分层剥离：①层 = 基础+个账（全国必有）】')
// 2026-09-15 记账利率订正（2021: 5.35% → 6.69%）后重算
const LAYS = [['15.00年', 2010, 12, 356.68, 68.16, 5.25],
              ['20.00年', 2005, 12, 517.95, 76.19, 6.82],
              ['25.00年', 2000, 12, 688.93, 84.13, 8.19]]
...
ok('  15年 ③层斜率 0.1777', +slopeOf(2010, 12, 'full').toFixed(4),0.1785, 0.0002)
ok('  38.42年 ③层斜率 0.4645', +slopeOf(1987, 7, 'full').toFixed(4),0.4659, 0.0002)
...
ok('  38.42年 ①层斜率 0.3332', +slopeOf(1987, 7, 'core').toFixed(4),0.3346, 0.0002)
...
ok('  月领多 2,601.69', +dTotalTier.toFixed(2),2614.39)
ok('  ⭐ 回本 12.29 年', +(dFeeTier / dTotalTier / 12).toFixed(2),12.23, 0.02)
ok('  ⭐ 回本年龄 72.54 岁', +(60.25 + dFeeTier / dTotalTier / 12).toFixed(2),72.48, 0.02)
const LIFE = 76.71
...
// 常量随记账利率订正同步：2026-09-15 由 388.41 / 67.82 更新为 389.28 / 68.16
const got讲故事IsLive = Math.abs(EFF_Y - 389.28) < 0.01 && Math.abs(EFF_T - 68.16) < 0.01
