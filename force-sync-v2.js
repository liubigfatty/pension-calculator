// 强制重新同步：先删除已有 avg_salary_history 块，再插入
const fs = require('fs');
...
jsonFiles.forEach(f => {
  ...
  try {
    ...
    if (avgIdx !== -1) {
      ...
      while (deleteStart > 0 && jsContent[deleteStart] !== '\n' && jsContent[deleteStart] !== ',') deleteStart--;
      if (jsContent[deleteStart] === ',') deleteStart++; // 跳过逗号
      else if (jsContent[deleteStart] === '\n') deleteStart++; // 跳到下一行开头
